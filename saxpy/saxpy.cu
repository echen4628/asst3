...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaMemcpy(device_result, resultarray, sizeof(float) * N, cudaMemcpyHostToDevice);
    ...
    double KernalDuration = kernalEndTime - kernalStartTime;
    printf("Kernal execution duration by CUDA saxpy: %.3f ms\n", 1000.f * KernalDuration);
